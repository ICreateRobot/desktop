...
function deletePhoto(button) {
    ...
    for (let index = data.length - 1; index >= 0; index--) {
        ...
        if (da.landmarks == pd && da.label == lab) {
            data.splice(index, 1);
        }
    }
    ...
    for (let index = imageDATA.length - 1; index >= 0; index--) {
        ...
        if (con.data.landmarks == pd && con.data.label == lab) {
            imageDATA.splice(index, 1);
        }
    }
...
function deleteSample(el) {
    ...
    relation.forEach(rela => {
        relationMap.set(rela.img, {
            landmarks: rela.landmarks,
            label: rela.label
        });
    });

    // 记录本次需要删除的 landmarks + label
    ...
    photoList.forEach(photo => {
        ...
        if (rela) {
            const key = `${rela.landmarks}__${rela.label}`;
            deleteKeys.add(key);
        }
    });
    ...
    // 删除 data 中符合条件的数据
    data = data.filter(da => {
        const key = `${da.landmarks}__${da.label}`;
        return !deleteKeys.has(key);
    });

    // 删除 imageDATA 中符合条件的数据
    imageDATA = imageDATA.filter(con => {
        const key = `${con.data.landmarks}__${con.data.label}`;
        return !deleteKeys.has(key);
    });
